import React from 'react';
import { Navbar } from '@/components/Navbar';
import { HeroSection } from '@/components/HeroSection';
import { FleetCatalog } from '@/components/FleetCatalog';
import { ServicesSection } from '@/components/ServicesSection';
import { PricingSection } from '@/components/PricingSection';
import { WhyChooseUs } from '@/components/WhyChooseUs';
import { HowItWorks } from '@/components/HowItWorks';
import { TermsAndFaq } from '@/components/TermsAndFaq';
import { ContactSection } from '@/components/ContactSection';
import { Footer } from '@/components/Footer';
import { COMPANY_INFO } from '@/data/info';

export default function Home() {
  return (
    <main className="min-h-screen bg-white text-slate-900">
      {/* Navigasi Header */}
      <Navbar />

      {/* 1. Hero & Booking Instan */}
      <HeroSection />

      {/* 2. Katalog Armada & Kategori */}
      <FleetCatalog />

      {/* 3. Daftar Tarif & Paket */}
      <PricingSection />

      {/* 4. Solusi Transportasi & Layanan */}
      <ServicesSection />

      {/* 5. Keunggulan Utama */}
      <WhyChooseUs />

      {/* 6. Cara Sewa 1-2-3 */}
      <HowItWorks />

      {/* 7. Testimoni, Syarat & FAQ */}
      <TermsAndFaq />

      {/* 8. Hubungi Kami & Formulir Pesan */}
      <ContactSection />

      {/* Footer */}
      <Footer />

      {/* Floating WhatsApp Button */}
      <a
        href={`https://wa.me/${COMPANY_INFO.whatsapp}?text=Halo%20RF%20Rental%20Mobil%20Bandung,%20saya%20ingin%20tanya%20ketersediaan%20unit%20mobil.`}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat WhatsApp"
        className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-[#25D366] hover:bg-[#1ebe5d] text-white rounded-full shadow-lg flex items-center justify-center transition-transform hover:scale-110 active:scale-95"
      >
        <svg className="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
          <path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.112 1.522 5.84L.057 23.882a.75.75 0 0 0 .921.921l6.055-1.466A11.945 11.945 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.75a9.695 9.695 0 0 1-4.932-1.348l-.353-.21-3.658.886.902-3.573-.23-.368A9.694 9.694 0 0 1 2.25 12C2.25 6.615 6.615 2.25 12 2.25S21.75 6.615 21.75 12 17.385 21.75 12 21.75z"/>
        </svg>
      </a>
    </main>
  );
}
